...
export async function addSession(user_id: string, session: AddSessionInput) {
  const { amount, pot_id, session_length, created_at, location } = session;
  ...
  return prisma.sessions.create({
    data: {
      ...
      created_at,
      total,
      top_ups_total: potMostRecentSession?.top_ups_total,
      location,
    },
  });
}
...
export async function getSessionsSinceLastChop(pot_id: number) {
  ...
  const sessions = await prisma.sessions.findMany({
    where: {
      pot_id: pot_id,
      AND: { created_at: { gt: lastChop?.created_at } },
    },
  });
  return sessions;
}
...
export async function deleteSession(session_id: number) {
  ...
  if (!(await canDeleteSession(session_id, session.user_id)))
