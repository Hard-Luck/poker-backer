...
import {
  ...
  canDeleteSession,
  deleteSession,
  getSessionWithComments,
  getSessionsSinceLastChop,
...
export const sessionRouter = createTRPCRouter({
  ...
  delete: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { id } = input;
      if (!(await canDeleteSession(id, user_id)))
        throw new TRPCError({ code: 'UNAUTHORIZED' });
      return deleteSession(id);
    }),
  createComment: privateProcedure
    .input(z.object({ session_id: z.number(), comment: z.string().min(2) }))
    .mutation(async ({ input, ctx }) => {
      const user_id = ctx.currentUser;
  ...
  deleteComment: privateProcedure
    .input(z.object({ user_id: z.string(), comment_id: z.number() }))
    .mutation(async ({ input, ctx }) => {
      const user_id = ctx.currentUser;
  ...
  sessionsSinceLastChop: privateProcedure
    .input(
      z.object({
        pot_id: z.number(),
      })
    ...
    .query(async ({ input }) => {
      ...
      const playedSessions = sessions.filter(
        session => session.transaction_type !== 'top_up'
      );
