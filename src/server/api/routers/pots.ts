...
export const potsRouter = createTRPCRouter({
  getCurrentUserPot: privateProcedure.query(async ({ ctx }) => {
  ...
  getIsBackerOfPot: privateProcedure
    .input(z.object({ pot_id: z.number() }))
    .query(async ({ ctx, input }) => {
      const id = ctx.currentUser;
      const { pot_id } = input;
  ...
  getChops: privateProcedure
    .input(z.object({ pot_id: z.number() }))
    .query(async ({ input }) => {
      const history = await getChopHistory(input.pot_id);
      return history;
    }),
  topUp: privateProcedure
    .input(z.object({ pot_id: z.number(), amount: z.number() }))
    .mutation(async ({ ctx, input }) => {
  ...
  delete: privateProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const { pot_id } = input;
      const user_id = ctx.currentUser;
      if (!(await isPotOwner(pot_id, user_id)))
